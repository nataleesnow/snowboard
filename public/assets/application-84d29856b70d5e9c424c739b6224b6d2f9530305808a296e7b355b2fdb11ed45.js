...
/*! Magnific Popup - v1.1.0 - 2016-03-13
...
(function() {
  ...
  Index = (function() {
    var getModal, reqSave, validWTP;

    function Index() {
      this.supplyWtp = bind(this.supplyWtp, this);
      this.saveWtp = bind(this.saveWtp, this);
      $(document).on('click', '.supply_wtp', this.supplyWtp).on('click', '.save_wtp', this.saveWtp);
    }

    Index.prototype.saveWtp = function(event) {
      var data, lift_pass_id, path, target, wtp_number;
      event.preventDefault();
      target = $(event.target);
      lift_pass_id = target.data("liftPassId");
      wtp_number = [];
      $.each($(".wtp_" + lift_pass_id), function(i, e) {
        if (e.value.match(/\w+/)) {
          return wtp_number.push(e.value);
        }
      });
      data = {
        lift_pass_id: lift_pass_id,
        wtp_number: wtp_number
      };
      path = $(event.target).attr("href");
      if (validWTP(data.lift_pass_id, data.wtp_number)) {
        return reqSave(data, path);
      }
    };

    Index.prototype.supplyWtp = function(event) {
      var lift_pass_id, modal;
      event.preventDefault();
    ...
    validWTP = function(lift_pass_id, wtp) {
      var modal;
      modal = getModal(lift_pass_id);
      modal.find(".callout.alert").html('').hide();
      if ($.isEmptyObject(wtp)) {
        if ($.isEmptyObject(wtp)) {
          modal.find(".callout.alert").html('Please enter WTP number.').show();
        }
        return false;
      }
      return true;
